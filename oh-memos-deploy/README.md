...
| 🕸�?**Knowledge Graph** | Link related memories for better context |
...
| "还记�?, "remember", "recall" | Searches specific memory |
| "为什�?, "why did we", "原因" | Searches decisions |
...
```bash
oh-memos-init                           # Initialize project
oh-memos-save "content" -t TYPE         # Save memory
oh-memos-search "query"                 # Search memories
```

### Memory Type Options

```bash
oh-memos-save "Fixed login bug" -t BUGFIX
oh-memos-save "Chose PostgreSQL" -t DECISION
oh-memos-save "Auth system complete" -t MILESTONE
oh-memos-save "Docker needs 2GB RAM" -t GOTCHA
oh-memos-save "Retry decorator pattern" -t CODE_PATTERN
oh-memos-save "Connection error fix" -t ERROR_PATTERN
```
...
└── scripts/
    ├── oh-memos_init_project.py   # Initialize project cube
    ├── oh-memos_save.py           # Save memory
    ├── oh-memos_search.py         # Search memories
    ├── linux/
    �?  ├── install.sh
    �?  ├── oh-memos-init.sh
    �?  ├── oh-memos-save.sh
...
|----------|---------|-------------|
| `oh-memos_URL` | `http://localhost:18000` | oh-memos API URL |
| `oh-memos_USER` | `dev_user` | Default user ID |
| `oh-memos_CUBES_DIR` | `~/.oh-memos_cubes` | Memory storage |
