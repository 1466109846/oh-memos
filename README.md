...
- Symbolic task canvases preserve in-flight work across context compaction.
- Wiki round-trip: exported Markdown can be edited and re-imported, making
  stored memories reviewable, repairable, and git-managed.
- Context-aware search, evidence packs, graph paths, and impact analysis expose
  both semantic and causal context.
- Project-path cube routing prevents cross-repository memory pollution.
- Markdown wiki export makes selected knowledge reviewable and versionable.
- PreToolUse and lifecycle hooks help assistants retrieve before editing and save
  important outcomes after work.
