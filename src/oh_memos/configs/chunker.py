...
class BaseChunkerConfig(BaseConfig):
    ...
    tokenizer_or_token_counter: str = Field(
        default="character", description="Tokenizer model name or a token counting function"
    )
...
class ChunkerConfigFactory(BaseConfig):
    ...
    config: dict[str, Any] | SerializeAsAny[BaseChunkerConfig] = Field(..., description="Configuration for the chunker backend")
