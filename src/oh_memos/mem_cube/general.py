...
from oh_memos.configs.utils import get_json_file_model_schema
from oh_memos.exceptions import ConfigurationError, MemCubeError
from oh_memos.log import get_logger
from oh_memos.mem_cube.base import BaseMemCube
from oh_memos.mem_cube.utils import apply_env_overrides, download_repo, merge_config_with_default
...
class GeneralMemCube(BaseMemCube):
    ...
    def act_mem(self) -> "BaseActMemory | None":
        ...
        if self._act_mem is None:
            logger.debug("Activation memory is not initialized. Returning None.")
    ...
    def para_mem(self) -> "BaseParaMemory | None":
        ...
        if self._para_mem is None:
            logger.debug("Parametric memory is not initialized. Returning None.")
    ...
    def pref_mem(self) -> "BaseTextMemory | None":
        ...
        if self._pref_mem is None:
            logger.debug("Preference memory is not initialized. Returning None.")
