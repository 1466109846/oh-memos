...
from oh_memos.api.handlers.config_builders import (
...
from oh_memos.mem_os.product_server import MOSServer
...
from oh_memos.memories.textual.prefer_text_memory.factory import (
    AdderFactory,
    ExtractorFactory,
    RetrieverFactory,
)
...
from oh_memos.vec_dbs.factory import VecDBFactory
...
def init_server() -> dict[str, Any]:
    ...
    if os.getenv("MEMSCHEDULER_USE_REDIS_QUEUE", "False").lower() == "true":
        try:
            from oh_memos.mem_scheduler.orm_modules.api_redis_model import APIRedisDBManager
    ...
    online_bot = None
    if dingding_enabled:
        from oh_memos.memos_tools.notification_service import get_online_bot_function
