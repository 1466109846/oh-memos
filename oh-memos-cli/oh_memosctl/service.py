...
def display_status(status: dict[str, ServiceStatus], mode_status: dict[str, tuple[ServiceStatus, int | None]] | None = None):
    ...
    status_icons = {
        ServiceStatus.RUNNING: "[green]â—?running[/green]",
        ServiceStatus.STOPPED: "[red]â—?stopped[/red]",
        ServiceStatus.ERROR: "[yellow]âš?error[/yellow]",
...
def start_services(modes: list[str] | None = None, project_dir: Path | None = None) -> bool:
    ...
    if status["neo4j"] != ServiceStatus.RUNNING:
        console.print("[yellow]âš?Neo4j is not running.[/yellow]")
    ...
    if status["qdrant"] != ServiceStatus.RUNNING:
        console.print("[yellow]âš?Qdrant is not running.[/yellow]")
        # Don't return False - let user decide
    ...
    for mode_name in modes:
        ...
        if pid is None:
            console.print(f"[red]âœ?Failed to start MCP for {mode_name}[/red]")
            success = False

    if success:
        console.print("[green]âœ?All MCP servers started[/green]")
...
def stop_services(modes: list[str] | None = None, project_dir: Path | None = None) -> bool:
    ...
    if success:
        console.print("[green]âœ?All MCP servers stopped[/green]")
